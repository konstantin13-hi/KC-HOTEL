...
export default function PhotosUploder({addedPhotos,onChange}){
    ...
    async function addPpotoBylink(event){
        event.preventDefault();
         const {data : filename} =   await axios.post('http://localhost:8080/upload-by-link',{link:photoLink})
         onChange(prev =>{
     ...
     <div className="flex gap-2">
                         ...
                         <div className="grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-col-6">
                            ...
                            {addedPhotos.length> 0 && addedPhotos.map(link=>(
                               ...
                               <button onClick={(ev) => removePhoto(ev,link) } className="cursor-pointer absolute bottom-1 right-1 text-white bg-black bg-opacity-50 p-1 rounded-2xl py-2 px-3">
                               ...
                               <button onClick={(ev) => selectAsMainPhoto(ev,link) } className="cursor-pointer absolute bottom-1 left-1 text-white bg-black bg-opacity-50 p-1 rounded-2xl py-2 px-3">
                               {link === addedPhotos[0] && (
